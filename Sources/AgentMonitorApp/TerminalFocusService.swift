...
enum TerminalFocusService {
    ...
    // Launch Services invokes its completion handler on a private background
    // queue. Keep this continuation bridge nonisolated so Swift does not add a
    // main-actor precondition to that callback (which would trap at runtime).
    nonisolated private static func openApplication(at url: URL, configuration: NSWorkspace.OpenConfiguration) async throws {
