...
`scripts/configure.py` merges Agent Monitor entries instead of replacing unrelated Codex or Claude hooks. Codex lifecycle hooks track activity, while a completion callback covers Codex desktop sessions. Existing notify commands are safely chained and restored on uninstall. Before writing, the script prints a unified diff and creates timestamped backups; it also repairs the recursive dispatcher created by affected older versions.
