...
`scripts/configure.py` merges Agent Monitor entries instead of replacing unrelated Codex or Claude hooks. Before writing, it prints a unified diff and creates timestamped backups. The uninstall path removes only commands containing `agent-monitor-helper` and restores a previously configured Codex `notify` command when applicable.
